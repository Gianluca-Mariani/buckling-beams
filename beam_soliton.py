    ...
    def ODE(self, t, x, args):
        ...
        def body(j, dxdt):
            ...
            dxdt = dxdt.at[4*j].set(
                jax.lax.cond(
                    j == 0,
                    lambda _: -self.xi**(4*j)*(self.A*(1-(-1)**i) + self.epsilon) * x[4*j] - self.xi**(4*j)*self.b*x[4*j]**3 - self.gamma*(2*x[4*j] - x0 - x[4*j+1]),
                    lambda _: -self.xi**(4*j)*(self.A*(1-(-1)**i) + self.epsilon) * x[4*j] - self.xi**(4*j)*self.b*x[4*j]**3 - self.gamma*(2*x[4*j] - x[4*j-1] - x[4*j+1]),
                    operand=None
                )
            )
            ...
            dxdt = dxdt.at[4*j+1].set(
                self.xi**(4*j+1)*(self.A*(1+(-1)**(i+1)) + self.epsilon) * x[4*j+1] - self.xi**(4*j+1)*self.b*x[4*j+1]**3 - self.gamma*(2*x[4*j+1] - x[4*j] - x[4*j+2])
            ) 

            # Connecting beam
            dxdt = dxdt.at[4*j+2].set(
                -self.xi**(4*j+2)*(self.A*(1-(-1)**(i+1)) + self.epsilon) * x[4*j+2] - self.xi**(4*j+2)*self.b*x[4*j+2]**3 - self.gamma*(2*x[4*j+2] - x[4*j+1] - x[4*j+3])
            )  
            ...
            dxdt = dxdt.at[4*j+3].set(
                jax.lax.cond(
                    j == self.n-1,
                    lambda _: self.xi**(4*j+3)*(self.A*(1+(-1)**i) + self.epsilon) * x[4*j+3] - self.xi**(4*j+3)*self.b*x[4*j+3]**3 - self.gamma*(x[4*j+3] - x[4*j+2]),
                    lambda _: self.xi**(4*j+3)*(self.A*(1+(-1)**i) + self.epsilon) * x[4*j+3] - self.xi**(4*j+3)*self.b*x[4*j+3]**3 - self.gamma*(2*x[4*j+3] - x[4*j+2] - x[4*j+4]),
                    operand=None
                )
            )
    ...
    def solve(self, i_stop, x0_array, i0=0, init="Down", print_b=False):
        ...
        for i in range(i_stop):
            ...
            solver_state = dfx.diffeqsolve(
                term, solver, t0=0.0, t1=100.0, dt0=1e-2, y0=y0,
                stepsize_controller=dfx.PIDController(rtol=1e-5, atol=1e-8), args=args
            )
            # Extract result
...
class gate:
    ...
    def ODE(self, t, x, args):
        ...
        #Connecting beam A
        dxdt0 = -(self.A*(1-(-1)**i) + self.epsilon) * x[0] - self.b*x[0]**3 - self.gamma*(2*x[0] - x0A - x[2])

        #Connecting beam B
        dxdt1 = -(self.A*(1-(-1)**i) + self.epsilon) * x[1] - self.b*x[1]**3 - self.gamma*(2*x[1] - x0B - x[2])

        #Gate (encoding) beam
        dxdt2 = (self.A*(1+(-1)**(i+1)) + self.epsilon) * x[2] - self.b*x[2]**3 - self.gamma*(3*x[2] - x[0] - x[1]- x[3]) - self.F

        #Connecting beam
        dxdt3 = -(self.A*(1-(-1)**(i+1)) + self.epsilon) * x[3] - self.b*x[3]**3 - self.gamma*(2*x[3] - x[2] - x[4])

        #Encoding beam
        dxdt4 = (self.A*(1+(-1)**i) + self.epsilon) * x[4] - self.b*x[4]**3 - self.gamma*(x[4] - x[3])
...
x0 = jnp.where((i_vals % 4 == 0) | (i_vals % 4 == 3), -jnp.sqrt((2*A+epsilon)/b), jnp.sqrt((2*A+epsilon)/b))
