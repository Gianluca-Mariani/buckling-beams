...
def test_solve():
    ...
    assert (
        sol.ys.shape[0] == sol.ts.size
    ), "Solution ys should match the time array length"
...
def test_time_evolution():
    ...
    assert (
        sol.ts[-1] == t_cycles * 2 * jnp.pi / omega
    ), f"Time should end at {t_cycles * jnp.pi / omega}"
...
def test_parameter_sensitivity():
    ...
    assert not jnp.allclose(
        sol1.ys, sol2.ys
    ), "Solution should differ for different omega"
